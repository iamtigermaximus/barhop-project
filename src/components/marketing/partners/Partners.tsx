...
const Partners = () => {
  ...
  useEffect(() => {
    const fetchBars = async () => {
      try {
        ...
        // Fetch all active bars (you can add filters if needed)
        const response = await fetch("/api/bars?limit=200");
        
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(errorData.error || `Failed to fetch bars: ${response.status}`);
        }

        const barsData: Bar[] = await response.json();
        console.log(`Fetched ${barsData.length} bars from API`);
  ...
  const sortedCategories = Object.entries(barsByCategory)
    .sort(([, a], [, b]) => b.length - a.length)
    .map(([category, categoryBars]) => ({
      category,
      bars: categoryBars.sort((a, b) => a.name.localeCompare(b.name)),
    }));
  ...
  const uniqueCities = new Set(bars.map((bar) => bar.city?.name).filter(Boolean));
  
  return (
    <Page>
      <Title>Our Partners</Title>
      <Description>
        Discover {bars.length} premium venues across {uniqueCities.size} cities
      </Description>
              ...
              {categoryBars.slice(0, 5).map((bar) => (
                <BarCard key={bar.id} href={`/app/bars/${bar.id}`}>
                  ...
                  <BarInfo>
                    <BarName>{bar.name}</BarName>
                    <BarLocation>
                      {bar.city?.name || "Unknown"} {bar.district && `• ${bar.district}`}
                    </BarLocation>
                    <BarDetails>
                      {bar.isOpen !== undefined && (
                        <StatusBadge $isOpen={bar.isOpen}>
                      ...
                      {bar.vipEnabled && <VIPBadge>VIP Access</VIPBadge>}
                    </BarDetails>
                  </BarInfo>
                </BarCard>
              ))}
