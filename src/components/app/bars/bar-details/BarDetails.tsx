...
// ============================================
// ALL STYLED COMPONENTS WITH THEME
// ============================================
...
const VerifiedTag = styled(Tag)`
  background: #22c55e;
  color: white;
`;

const HotspotTag = styled(Tag)`
  background: rgba(249, 115, 22, 0.2);
  color: #f97316;
  border: 1px solid rgba(249, 115, 22, 0.3);
`;

const SocialTag = styled(Tag)`
  background: rgba(59, 130, 246, 0.2);
  color: #3b82f6;
`;
...
const TodayHoursText = styled.span`
  color: ${({ theme }) => theme.colors.textMuted};
  font-size: 0.9rem;
  font-family: ${({ theme }) => theme.fonts.dm};
`;
...
const Tab = styled.button<{ $isActive: boolean }>`
  ...
  border-bottom: 2px solid
    ${(props) =>
      props.$isActive ? props.theme.colors.secondaryAccent : "transparent"};
  transition: all 0.3s ease;
  font-family: ${({ theme }) => theme.fonts.dm};
...
const SocialActivityTitle = styled.h3`
  color: ${({ theme }) => theme.colors.textPrimary};
  margin-bottom: 1rem;
  font-family: ${({ theme }) => theme.fonts.dm};
`;
...
const HoursValue = styled.span<{ $isClosed?: boolean }>`
  color: ${(props) => (props.$isClosed ? "#ef4444" : "#22c55e")};
  font-weight: 600;
  font-family: ${({ theme }) => theme.fonts.mono};
`;
...
const VipPassesGrid = styled.div`
  display: grid;
  grid-template-columns: repeat(auto-fill, minmax(320px, 1fr));
  gap: 1.5rem;
`;

const VipPassCard = styled.div`
  background: ${({ theme }) => theme.colors.secondaryBackground};
  border-radius: 16px;
  padding: 1.5rem;
  border: 1px solid ${({ theme }) => theme.colors.border};
...
const VipPassHeader = styled.div`
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  margin-bottom: 1rem;
...
const VipPassDescription = styled.p`
  color: ${({ theme }) => theme.colors.textMuted};
  margin-bottom: 1rem;
  line-height: 1.5;
  font-size: 0.9rem;
...
const VipPassBenefits = styled.div`
  margin-bottom: 1rem;
`;
...
const VipPassDetails = styled.div`
  display: flex;
  flex-wrap: wrap;
  gap: 0.75rem;
  margin-bottom: 1.5rem;
...
const BuyVipButton = styled.button`
  width: 100%;
  background: linear-gradient(135deg, #8b5cf6, #6d28d9);
  color: white;
  border: none;
...
const BarDetails = ({ barId }: BarDetailsProps) => {
  ...
  const fetchBar = useCallback(async (): Promise<void> => {
    try {
      ...
      const response = await fetch(`/api/bars/${barId}`);
      if (!response.ok) {
        if (response.status === 404) throw new Error("Bar not found");
        throw new Error(`Failed to fetch bar: ${response.status}`);
      }
      const barData: BarData = await response.json();
      setBar(barData);
  ...
  const handleVipClick = (): void => setActiveTab("vip");
  const handlePhotoClick = (url: string): void => setSelectedPhoto(url);
  const handleCloseModal = (): void => setSelectedPhoto(null);
    ...
    <Container>
      ...
      <Header>
        ...
        <BarMeta>
          ...
          <Tag>{bar.type.replace(/_/g, " ")}</Tag>
          {bar.vipEnabled && <VipTag>VIP Available</VipTag>}
          {bar.isVerified && <VerifiedTag>✓ Verified</VerifiedTag>}
          {bar.socialActivity?.isHotspot && <HotspotTag>🔥 Hotspot</HotspotTag>}
        </BarMeta>
        <Address>{bar.address}</Address>
      ...
      <ContentGrid>
        <MainContent>
          {activeTab === "overview" && (
            <>
              ...
              {bar.socialActivity && (
                <SocialActivityPanel>
                  <SocialActivityTitle>Live Activity</SocialActivityTitle>
                  <ActivityStats>
                    <ActivityStat>
                      <StatIcon>👥</StatIcon>
                      <StatContent>
                        ...
                        <StatLabel>People here</StatLabel>
                      </StatContent>
                    </ActivityStat>
                    <ActivityStat>
                      <StatIcon>🎯</StatIcon>
                      <StatContent>
                        ...
                        <StatLabel>Active meetups</StatLabel>
                      </StatContent>
                    </ActivityStat>
                    {bar.socialActivity.heatLevel > 0 && (
                      <ActivityStat>
              ...
              {bar.operatingHours && (
                <OperatingHoursTable>
                  ...
                  {Object.entries(bar.operatingHours).map(([day, hours]) => (
                    <HoursRow key={day}>
                      <DayName>{day}</DayName>
                      <HoursValue $isClosed={hours.open === "Closed"}>
                        {hours.open === "Closed"
                          ? "Closed"
                          : `${hours.open} - ${hours.close}`}
                      </HoursValue>
          ...
          {activeTab === "vip" && hasVipPasses && (
            <VipPassesSection>
              ...
              <StatLabel>Skip the line and enjoy exclusive benefits</StatLabel>
              <VipPassesGrid>
                {bar.vipPassesEnhanced.map((pass) => (
                  <VipPassCard key={pass.id}>
                    <VipPassHeader>
                      <VipPassName>{pass.name}</VipPassName>
                      <VipPassPrice>
                        <CurrentPrice>
                          {formatPrice(pass.priceCents)}
                        </CurrentPrice>
                    ...
                    <VipPassDescription>{pass.description}</VipPassDescription>
                    <VipPassBenefits>
                      {pass.benefits.slice(0, 3).map((benefit, index) => (
          ...
          {activeTab === "photos" && allImages.length > 0 && (
            <PhotosSection>
              ...
              <StatLabel>Explore the atmosphere of {bar.name}</StatLabel>
              {bar.coverImage && (
                <CoverPhoto onClick={() => handlePhotoClick(bar.coverImage!)}>
                  <img src={bar.coverImage} alt={`${bar.name} cover`} />
                </CoverPhoto>
              )}
              {bar.imageUrls && bar.imageUrls.length > 0 && (
        ...
        <Sidebar>
          ...
          <QuickInfoSection>
            <QuickInfoTitle>Quick Info</QuickInfoTitle>
            <InfoGrid>
              <InfoItem>
                <strong>📍 Location:</strong>
                <span>{bar.cityName || "Helsinki"}</span>
          ...
          {(bar.profileViews > 0 || bar.directionClicks > 0) && (
            <QuickInfoSection>
              <QuickInfoTitle>Engagement</QuickInfoTitle>
              <InfoGrid>
                {bar.profileViews > 0 && (
                  <InfoItem>
                    <strong>👁️ Profile Views:</strong>
