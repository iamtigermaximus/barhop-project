...
// New styled components with BottomNav.tsx styling - USING CORRECT THEME KEYS
const MobileBottomNav = styled.nav`
  ...
  border-top: 1px solid ${({ theme }) => theme.colors?.border || "#2d2d3f"};
  display: flex;
...
const SidebarNavItem = styled(Link)<{ $active?: boolean }>`
  display: flex;
  align-items: center;
  gap: 12px;
  padding: 12px 16px;
  ...
  color: ${({ $active, theme }) =>
    $active
      ? theme.colors?.primaryAccent || "#8b5cf6"
      : theme.colors?.textSecondary || "#94a3b8"};
  background: ${({ $active, theme }) =>
    $active ? `${theme.colors?.primaryAccent || "#8b5cf6"}15` : "transparent"};
  text-decoration: none;
  font-family: ${({ theme }) => theme.fonts?.dm || "system-ui"};
  font-size: 15px;
  font-weight: ${({ $active }) => ($active ? "600" : "400")};
  transition: all 0.2s;

  &:hover {
    background: ${({ theme }) => theme.colors?.tertiaryBackground || "#2d2d3f"};
...
const TopNavWrapper = styled.div<{ $hasSidebar?: boolean }>`
  @media (min-width: 768px) {
...
const AppNavbar = () => {
  ...
  // Route active state check - KEEPING YOUR ORIGINAL LOGIC
  const isActive = (path: string) => {
    if (path === "/app") return pathname === "/app" || pathname === "/app/";
    if (path === "/app/vip-pass")
      return pathname === "/app/vip-pass" || pathname === "/app/vip-pass/";
  ...
  // YOUR ORIGINAL DESKTOP NAVIGATION ITEMS
  const desktopNavigation = [
    { name: "Home", href: "/app", icon: FaHome },
    { name: "Social", href: "/app/social", icon: FaUsers },
    { name: "Bars", href: "/app/bars", icon: FaGlassMartiniAlt },
    { name: "VIP Passes", href: "/app/vip-pass", icon: FaCrown },
    { name: "Create Event", href: "/app/crawl-planner", icon: FaPlus },
    ...(session
      ? [{ name: "My Events", href: "/app/my-crawls", icon: FaCompass }]
      : []),
    ...(session
      ? [
          {
      ...
      : []),
    { name: "Discover Events", href: "/app/crawls-dashboard", icon: FaCompass },
  ];

  // YOUR ORIGINAL MOBILE NAVIGATION ITEMS
  const mobileNavigation = [
    { name: "Home", href: "/app", icon: FaHome },
    { name: "Social", href: "/app/social", icon: FaUsers },
    { name: "Create", href: "/app/crawl-planner", icon: FaPlus },
  ...
  if (isMobile) {
    return (
      <>
        {/* Your existing top navbar for mobile */}
        <Nav $isMobile={true}>
          <NavContainer>
            <Logo href="/app">
              <LogoIcon>
            ...
            >
              {isAuthenticated && (
                <>
                  <Link
                    href="/app/chat/my-chats"
                    style={{ padding: "0.5rem", color: "#e2e8f0" }}
                  >
                    <FaComments size={20} />
                  </Link>
                  <Link
        ...
        {/* New Mobile Bottom Navigation with YOUR menu items */}
        <MobileBottomNav>
          {mobileNavigation.map((item) => {
            const IconComponent = item.icon;
            ...
            return (
              ...
              >
                ...
                <span>
                  {item.requiresAuth && !session ? "Login" : item.name}
                </span>
              </MobileNavItem>
            );
          })}
  ...
  return (
    <>
      {/* Desktop Sidebar - Using YOUR menu items */}
      <DesktopSidebar>
        <SidebarLogo href="/app">
          <SidebarLogoIcon>
            <Image
              src="/hoppr-neon-nobg.png"
              alt="Hoppr Logo"
      ...
      {/* Your existing top navbar for desktop - adjusted with margin */}
      <TopNavWrapper $hasSidebar={true}>
        <Nav>
          <NavContainer>
            {/* Logo hidden on desktop since we have it in sidebar */}
            <div
              style={{
                width: "100%",
                display: "flex",
              ...
              <AuthSection>
                {isAuthenticated ? (
                  ...
                  <>
                    ...
                    <GradientButton href="/app/auth/signup">
                      Sign up
                    </GradientButton>
                  </>
                )}
